...
1. 创建一个可以访问 starred repositories 的 classic GitHub Personal Access Token。
...
- 完整同步和取消 Star：使用 classic personal access token，并勾选 `public_repo`。
- 需要访问私有 starred repositories：使用 classic personal access token，并勾选 `repo`。

创建 classic token：

1. 打开 [GitHub classic token 设置](https://github.com/settings/tokens/new)。
2. 填写 token 名称和过期时间。
3. 按使用范围二选一：
   - 只管理公开 starred repositories：勾选 `public_repo`。
   - 需要管理公开和私有 starred repositories：勾选 `repo`。
4. 不要勾选无关权限，例如 `workflow`、`write:packages`、`admin:org`、`gist`、`notifications`、`user`、`delete_repo`、`project` 或密钥管理相关权限。
5. 生成 token 后只复制一次，并粘贴到 StarMagpie。
6. 后续需要替换 token 时，使用 StarMagpie 的 **更新 GitHub Token** 按钮。这个操作只替换 Keychain 中的 token，不会删除本地仓库、备注和分类。
